    ...
    if path.suffix == ".whl":
        ...
        with tarfile.open(path, "r:gz") as archive:
            ...
            for item in infos:
                ...
                data = (
                    extracted.read()
                    if extracted is not None and item.size <= 2_000_000
                    else b""
                )
                total += item.size
            ...
            for item in archive.getmembers():
                if (item.issym() or item.islnk()) and not _safe_name(item.linkname):
                    errors.append(
                        f"unsafe archive link: {item.name} -> {item.linkname}"
                    )
    else:
...
def main(argv: list[str] | None = None) -> int:
    ...
    reports = [
        inspect(path, allowed, args.max_bytes, args.max_members) for path in paths
    ]
